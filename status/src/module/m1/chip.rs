...
fn calc_freq(item: CFDictionaryRef, freqs: &[u32]) -> (u32, f32) {
  ...
  // IDLE / DOWN for CPU; OFF for GPU; DOWN apenas em chips M2?/M3 Max
...
fn init_smc() -> WithError<(SMC, Vec<String>, Vec<String>)> {
  ...
  for name in &names {
    ...
    // Infelizmente, não se sabe quais chaves são responsáveis por quê.
    // Basicamente, no código que pode ser encontrado publicamente, "Tp" é usado para CPU e "Tg" para GPU.

    match name {
      // "Tp" – núcleos de desempenho, "Te" – núcleos de eficiência
...
impl Sampler {
  ...
  fn get_temp_smc(&mut self) -> WithError<TempMetrics> {
    ...
    let cpu_temp = zero_div(cpu_metrics.iter().sum::<f32>(), cpu_metrics.len() as f32);
    let gpu_temp = zero_div(gpu_metrics.iter().sum::<f32>(), gpu_metrics.len() as f32);

    Ok(TempMetrics { cpu_temp, gpu_temp })
  }
  ...
  fn get_temp_hid(&mut self) -> WithError<TempMetrics> {
    ...
    let cpu_temp = zero_div(cpu_values.iter().sum(), cpu_values.len() as f32);
    let gpu_temp = zero_div(gpu_values.iter().sum(), gpu_values.len() as f32);

    Ok(TempMetrics { cpu_temp, gpu_temp })
  }

  fn get_temp(&mut self) -> WithError<TempMetrics> {
    // HID para M1, SMC para M2/M3
    // ATUALIZAÇÃO: Parece que HID/SMC está relacionado à versão do SO, não ao chip (SMC disponível a partir do macOS 14)
  ...
  pub fn get_metrics(&mut self, duration: u32) -> WithError<Metrics> {
    ...
    // faz várias amostras para suavizar as métricas
      ...
      for x in sample {
        ...
        if x.group == "Energy Model" {
          match x.channel.as_str() {
            ...
            // "CPU Energy" para Básico / Max, "DIE_{}_CPU Energy" para Ultra
            c if c.ends_with("CPU Energy") => rs.cpu_power += cfio_watts(x.item, &x.unit, dt)?,
            // mesmo padrão para as próximas chaves: "ANE" para Básico, "ANE0" para Max, "ANE0_{}" para Ultra
